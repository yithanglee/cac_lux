defmodule LuxWeb.ShopProductTagControllerTest do
  ...
  describe "create shop_product_tag" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn =
        post(conn, Routes.shop_product_tag_path(conn, :create), shop_product_tag: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn =
        post(conn, Routes.shop_product_tag_path(conn, :create), shop_product_tag: @invalid_attrs)
  ...
  describe "edit shop_product_tag" do
    ...
    test "renders form for editing chosen shop_product_tag", %{
      conn: conn,
  ...
  describe "update shop_product_tag" do
    ...
    test "redirects when data is valid", %{conn: conn, shop_product_tag: shop_product_tag} do
      conn =
        put(conn, Routes.shop_product_tag_path(conn, :update, shop_product_tag),
          shop_product_tag: @update_attrs
        )
    ...
    test "renders errors when data is invalid", %{conn: conn, shop_product_tag: shop_product_tag} do
      conn =
        put(conn, Routes.shop_product_tag_path(conn, :update, shop_product_tag),
          shop_product_tag: @invalid_attrs
        )
