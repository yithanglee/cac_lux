defmodule LuxWeb.LoginController do
  use LuxWeb, :controller
  import Ecto.Query
  alias Lux.{Settings, Repo}

  def index(conn, params) do
    render(conn, "login.html", layout: {LuxWeb.LayoutView, "login.html"})
  end

  def register(conn, params) do
    render(conn, "register.html", layout: {LuxWeb.LayoutView, "login.html"})
  end
  ...
  def set_new_member_password(conn, %{"token" => token, "password" => password}) do
    ...
    map = Lux.Settings.decode_member_token(token)
    m = Lux.Settings.get_member!(map.id)

    {:ok, m} = Lux.Settings.update_member(m, %{crypted_password: crypted_password})
  ...
  def reset(conn, %{"token" => token} = params) do
    ...
    render(conn, "reset.html", token: token, layout: {LuxWeb.LayoutView, "login.html"})
  end
