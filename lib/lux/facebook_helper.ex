...
#   import Ecto.Query
#   import LuxWeb.Gettext
#   alias Lux.{Settings, Repo}
#   alias Settings.{ShopProduct, FacebookPage, PageVisitor}

#   @app_secret Application.get_env(:lux, :facebook)[:app_secret]
#   @app_id Application.get_env(:lux, :facebook)[:app_id]
...
#         {:ok, pv} =
#           Lux.Settings.create_page_visitor(%{
...
#       else
#         {:ok, pv} =
#           Lux.Settings.update_page_visitor(page_visitor_data, %{
#             facebook_page_id: fb_page_id
#           })
...
#     Lux.Settings.create_video_comment(%{
#       ms_id: comment.id,
...
#     LuxWeb.Endpoint.broadcast("facebook_page:#{pv.facebook_page_id}", "new_msg", %{
...
#     page_visitors_data =
#       Lux.Settings.get_live_video_by_fb_id(live_id)
...
#     page = Lux.Settings.get_facebook_page_by_pat(pat) |> List.first()
...
#                 "status" => status
#               } = video <- videos do
#             check = Lux.Settings.get_live_video_by_fb_id(live_id)
...
#               else
#                 {:ok, lv} =
#                   Lux.Settings.create_live_video(
#                     video
...
#           if live_now != nil do
#             check = Lux.Settings.get_live_video_by_fb_id(live_now["id"])
...
#   def get_live_video(live_now, page_access_token, %Lux.Settings.LiveVideo{} = live_video) do
...
#                 } = comment <- comments do
#               page_visitor =
#                 with pv <- Lux.Settings.get_page_visitor_by_psid(psid),
#                      true <- pv != nil do
#                   pv
#                 else
#                   _ ->
#                     {:ok, pv} =
#                       Lux.Settings.create_page_visitor(%{psid: psid, name: visitor_name})
...
#   def next_comment_pages(url, %Lux.Settings.LiveVideo{} = live_video) do
#     res = HTTPoison.get(url)
...
#             else
#               _ ->
#                 {:ok, pv} = Lux.Settings.create_page_visitor(%{psid: psid, name: visitor_name})
#                 pv
#             end
...
#     user = Lux.Settings.get_user_by_fb_user_id(user_id)
...
#             {:ok, page} =
#               Lux.Settings.create_facebook_page(%{
#                 user_id: user.id,
#                 page_id: fb_page["id"],
#                 name: fb_page["name"],
...
#     base_url = Application.get_env(:lux, :facebook)[:base_url]
#     # visitor.lang
...
#           existing_carts = Lux.Settings.get_psid_orders(page_visitor)
...
#         page = Lux.Repo.get_by(Lux.Settings.FacebookPage, page_id: fb_page_id)

#         if page != nil do
#           set_ps = fn sender_psid ->
#             page_visitor =
#               Lux.Repo.get_by(Lux.Settings.PageVisitor,
...
#                     })
#                   else
#                     Lux.Settings.update_page_visitor(existing_page_commenter, %{
#                       facebook_page_id: page.id
#                     })
#                   end
