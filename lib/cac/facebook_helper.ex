...
#       else
#         {:ok, pv} =
#           Cac.Settings.update_page_visitor(page_visitor_data, %{
#             facebook_page_id: fb_page_id
#           })
...
#     Cac.Settings.create_video_comment(%{
#       ms_id: comment.id,
...
#     CacWeb.Endpoint.broadcast("facebook_page:#{pv.facebook_page_id}", "new_msg", %{
...
#     page_visitors_data =
#       Cac.Settings.get_live_video_by_fb_id(live_id)
...
#     page = Cac.Settings.get_facebook_page_by_pat(pat) |> List.first()
...
#                 "status" => status
#               } = video <- videos do
#             check = Cac.Settings.get_live_video_by_fb_id(live_id)
...
#               else
#                 {:ok, lv} =
#                   Cac.Settings.create_live_video(
#                     video
...
#           if live_now != nil do
#             check = Cac.Settings.get_live_video_by_fb_id(live_now["id"])
...
#   def get_live_video(live_now, page_access_token, %Cac.Settings.LiveVideo{} = live_video) do
...
#                 } = comment <- comments do
#               page_visitor =
#                 with pv <- Cac.Settings.get_page_visitor_by_psid(psid),
#                      true <- pv != nil do
#                   pv
#                 else
#                   _ ->
#                     {:ok, pv} =
#                       Cac.Settings.create_page_visitor(%{psid: psid, name: visitor_name})
...
#   def next_comment_pages(url, %Cac.Settings.LiveVideo{} = live_video) do
#     res = HTTPoison.get(url)
...
#             else
#               _ ->
#                 {:ok, pv} = Cac.Settings.create_page_visitor(%{psid: psid, name: visitor_name})
#                 pv
#             end
